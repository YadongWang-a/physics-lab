...
async function smokeEmptyWs(): Promise<void> {
  ...
  const deadline = Date.now() + 45000
  let state = { rail: false, dialog: false, items: 1, footEntry: false, listHint: false, newBtn: false }
  while (Date.now() < deadline) {
    state = await win.webContents.executeJavaScript(
      `(() => ({
        ...
        newBtn: [...document.querySelectorAll('button')].some((b) => b.textContent?.trim() === '新建')
      }))()`
    )
    if (state.rail && state.dialog && state.footEntry && state.listHint && state.newBtn) break
    await delay(500)
  }
  const ok = state.rail && state.dialog && state.items === 0 && state.footEntry && state.listHint && state.newBtn
  console.log(`[smoke-emptyws] rail=${state.rail}; dialog=${state.dialog}; items=${state.items}; footEntry=${state.footEntry}; listHint=${state.listHint}; newBtn=${state.newBtn}`)
  app.exit(ok ? 0 : 1)
}
