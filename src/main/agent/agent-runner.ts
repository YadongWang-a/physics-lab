 ...
 * 结束（401/限流/超时等）时返回错误文本，否则 null。
 * SDK 的 agent_settled 事件不携带错误（只写在消息上），应用层必须自查。
 */
export function lastTurnError(messages: readonly unknown[]): string | null {
  const last = messages[messages.length - 1] as
    | { role?: string; stopReason?: string; errorMessage?: string }
    | undefined
  if (last?.role === 'assistant' && last.stopReason === 'error') {
    return last.errorMessage && last.errorMessage.trim() ? last.errorMessage : '模型调用失败（回合以错误结束）'
  }
  return null
}
