...
| **写入校验** | `write_demo`/`edit_demo`/`validate_demo` 三个自定义工具（ADR 0011）：写入前主进程校验——命名 kebab-case、作用域（只写目标文件/改名）、结构（DOCTYPE/demo-mode/lib 引用/配对）、`physics-demo` 标记、内联 JS 语法为硬错误；浅色违规与形态要素为非阻塞警告 |
...
- [0007-present-mode.md](./docs/adr/0007-present-mode.md) — 演示模式架构（已废弃：功能取消）
