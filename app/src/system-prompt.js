...
## 【问题理解】
复述老师要演示的物理问题、涉及的核心量与目标。**动笔前先自检以下三问**（这是生成正确性的前置闸门，写完 HTML 才发现理解错就晚了）：

1. **形态判定**：本题是时间演化类（运动、过程）-> 动态，还是平衡/受力分析/几何关系类 -> 静态？一句话给结论 + 理由。（与 CONVENTIONS §0、§4⑧ 一致，决定后面是否做动画。）
2. **核心量与可调参数**：这道题演示的是哪几个物理量之间的关系？用户能调哪些参数？要观测什么？（抓错量 = 整个演示偏。）
3. **关键假设**：题目有无模糊处需假设？如"碰撞"未说弹性/非弹性、"斜面"未说有无摩擦、"轻绳"是否计质量。有歧义就在此**明确标注你的假设**再继续；无歧义的简单题（如自由落体求速度）一句话带过即可，不必硬凑。

简单题三问可简述，但**第 1 问（形态判定）和第 2 问（核心量）必须显式给出**--这两个一旦答错，推导和演示全错。
